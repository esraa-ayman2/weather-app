        ...
        return {
            ...
            async transform(code, id) {
                ...
                if (TS_EXT_REGEX.test(id)) {
                    ...
                    // console.log('[ANALOG DEBUG]', id);
                    const typescriptResult = await fileEmitter?.(id);
                    // console.log('[ANALOG DEBUG RESULT]', id, !!typescriptResult, typescriptResult?.content?.length);
...
export function createFileEmitter(program, transformers = {}, onAfterEmit, angularCompiler, liveReload, disableTypeChecking) {
    return async (file, stale) => {
        ...
        if (!sourceFile) {
            ...
            const matchingRoot = rootFiles.find(rf => rf === file || file.endsWith(rf) || rf.endsWith(file) || file.includes(rf) || rf.includes(file));
            if (matchingRoot) {
                sourceFile = program.getSourceFile(matchingRoot);
            }
        }
        if (!sourceFile) {
            const allFiles = program.getSourceFiles();
            sourceFile = allFiles.find(sf => sf.fileName === file || sf.fileName.endsWith(file) || file.endsWith(sf.fileName) || file.includes(sf.fileName) || sf.fileName.includes(file));
        }
        if (!sourceFile) {
            console.error('[ANALOG] Could not find sourceFile for:', file, 'Root files:', program.getRootFileNames());
            return undefined;
        }
        if (stale) {
